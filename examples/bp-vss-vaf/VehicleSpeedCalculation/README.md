...
This is a VAF application module project: VehicleSpeedCalculation
An application module project can do the following:
...
Configure the project in the .py file and generate the database in JSON format
using VAF CaC (Configuration as Code) support:
``` bash
vaf model generate
```

Update the existing model artifacts (from related interface projects):
``` bash
vaf model update
```

Import from an existing interface project:
``` bash
vaf project import
```
